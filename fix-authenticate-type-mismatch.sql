-- Fix authenticate_user function type mismatch error
-- This script fixes the VARCHAR(50) vs TEXT type mismatch

-- 1. Disable RLS on users table to ensure access
...
-- 3. Ensure users table has all required columns with correct types
DO $$ 
BEGIN
    -- Add department column if it doesn't exist (VARCHAR(100) to match function)
    ...
    ELSE
        -- Ensure department is VARCHAR(100) not VARCHAR(50)
        ALTER TABLE public.users ALTER COLUMN department TYPE VARCHAR(100);
    END IF;
...
-- 4. Clear existing users and create fresh ones
DELETE FROM public.users;

-- Insert all 7 users with proper roles (using password_hash column)
...
-- 5. Drop and recreate authenticate_user function with proper type casting
...
-- 7. Show final user count and verify table structure
SELECT COUNT(*) as total_users FROM public.users;
SELECT username, full_name, role, is_authorized, department FROM public.users ORDER BY username;

-- 8. Test a single login to verify the function works
SELECT 'Testing admin login:' as test_info;
