-- =====================================================
-- ULTIMATE DATABASE FIX - BULLETPROOF VERSION
-- Smart Supply Chain & Procurement Management System
-- =====================================================
-- This script handles ALL edge cases and column dependencies

-- Step 1: Drop existing function to avoid conflicts
-- =====================================================
DROP FUNCTION IF EXISTS authenticate_user(TEXT, TEXT);

-- Step 2: Fix users table completely
-- =====================================================
...
-- Step 7: Create authenticate_user function
-- =====================================================
...
-- Step 8: Insert/Update admin user
-- =====================================================
...
-- Step 9: Create indexes (only after columns exist)
-- =====================================================
...
BEGIN
    -- Documents indexes
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'documents' AND column_name = 'category') THEN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'assets' AND column_name = 'criticality') THEN
        CREATE INDEX IF NOT EXISTS idx_assets_criticality ON assets(criticality);
    END IF;
    
    -- Maintenance logs indexes
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'maintenance_logs' AND column_name = 'asset_id') THEN
        CREATE INDEX IF NOT EXISTS idx_maintenance_logs_asset_id ON maintenance_logs(asset_id);
    END IF;
    
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'maintenance_logs' AND column_name = 'status') THEN
        CREATE INDEX IF NOT EXISTS idx_maintenance_logs_status ON maintenance_logs(status);
    END IF;
    
    -- Inventory indexes
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'inventory' AND column_name = 'category') THEN
...
BEGIN
    ...
    -- Count enhanced columns
    SELECT COUNT(*) INTO column_count
    FROM information_schema.columns
    WHERE table_schema = 'public'
    AND table_name IN ('documents', 'assets', 'maintenance_logs')
    AND column_name IN ('category', 'description', 'version', 'criticality', 'priority');
    
    RAISE NOTICE '✅ ULTIMATE DATABASE FIX COMPLETE!';
    RAISE NOTICE '✅ % essential tables are ready', table_count;
    RAISE NOTICE '✅ % enhanced columns added', column_count;
