-- Fix Inventory Schema - Final Version
-- This script handles existing tables and avoids duplicate publication errors

-- Add unit_price column to inventory table (if not exists)
ALTER TABLE public.inventory 
ADD COLUMN IF NOT EXISTS unit_price DECIMAL(10,2) DEFAULT 0;

-- Add created_by column to inventory table (if not exists)
ALTER TABLE public.inventory 
ADD COLUMN IF NOT EXISTS created_by UUID REFERENCES public.users(id) ON DELETE SET NULL;

-- Create inventory_approvals table for approval workflow (if not exists)
...
-- Enable realtime for inventory_approvals table (only if not already added)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_publication_tables 
    WHERE pubname = 'supabase_realtime' 
    AND tablename = 'inventory_approvals'
  ) THEN
...
-- Insert sample inventory approval data (only if table is empty)
INSERT INTO public.inventory_approvals (
  inventory_id,
  item_name,
  quantity,
...
WHERE NOT EXISTS (SELECT 1 FROM public.inventory_approvals LIMIT 1);
