-- Fix users table - Add missing password column and create users
-- This script will add the password column and create all users
...
-- 3. Check if password_hash column exists and handle it
DO $$ 
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns 
               WHERE table_name = 'users' AND column_name = 'password_hash') THEN
        RAISE NOTICE 'password_hash column exists - will use it for authentication';
    ELSIF EXISTS (SELECT 1 FROM information_schema.columns 
                  WHERE table_name = 'users' AND column_name = 'password') THEN
        RAISE NOTICE 'password column exists - will use it for authentication';
    ELSE
...
-- 6. Drop and recreate authenticate_user function
...
-- 7. Test the authenticate_user function
DO $$
DECLARE
    test_result RECORD;
